...
let () =
  ...
  print_string "Union: ";
  foreach (union s1 s2) (fun x -> Printf.printf "%d " x);
  print_endline "";

  print_string "Intersection: ";
  foreach (inter s1 s2) (fun x -> Printf.printf "%d " x);
  print_endline "";

  print_string "Difference (s1 - s2): ";
  foreach (diff s1 s2) (fun x -> Printf.printf "%d " x);
  print_endline "";

  let bound = bind s1 (fun x -> [x; x * 10]) in
  print_string "Bind (x and x*10): ";
  foreach bound (fun x -> Printf.printf "%d " x);
  print_endline "";

  Printf.printf "For all elements > 0 in s1? %b\n" (for_all s1 (fun x -> x > 0));
